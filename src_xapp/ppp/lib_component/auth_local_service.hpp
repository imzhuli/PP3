...
class xAuthLocalService final : public xAuthServiceStub {
public:
    bool        Init(const std::string & AuthFilePath);
    void        Clean();
    void        Tick(uint64_t NowMS);
    std::string OutputAudit() const;
    void        BindAuditService(xAuditServiceStub * Service) { AuditService = Service; }
...
private:
    ...
    //
    xAuditServiceStub *                       AuditService;
