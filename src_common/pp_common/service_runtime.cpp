...
static void InitLogger() {
    ...
    SERVICE_RUNTIME_ASSERT(static_cast<xBaseLogger *>(Logger)->Init(std::string(ServiceEnvironment.DefaultLoggerFilePath).c_str()));
    Logger->SetLogLevel(eLogLevel::Debug);

    AuditLogger = new xBaseLogger();
    SERVICE_RUNTIME_ASSERT(static_cast<xBaseLogger *>(AuditLogger)->Init(std::string(ServiceEnvironment.DefaultAuditLoggerFilePath).c_str()));
}

static void CleanLogger() {
    SERVICE_RUNTIME_ASSERT(AuditLogger);
    static_cast<xBaseLogger *>(AuditLogger)->Clean();
    delete Steal(AuditLogger, &NonLogger);

    SERVICE_RUNTIME_ASSERT(Logger);
...
void AssertServiceEnviromentReady() {
    SERVICE_RUNTIME_ASSERT(Instance);
}
...
xServiceEnvironmentGuard::xServiceEnvironmentGuard(int argc, char ** argv)
    ...
    SERVICE_RUNTIME_ASSERT(ServiceIoContext->Init());
    SERVICE_RUNTIME_ASSERT(ServiceRunState.Start());
    Instance = this;
}
...
xServiceEnvironmentGuard::~xServiceEnvironmentGuard() {
    auto G = std::lock_guard(EnvMutex);
    SERVICE_RUNTIME_ASSERT(this == Instance);
